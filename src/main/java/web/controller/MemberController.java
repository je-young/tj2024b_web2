...
public class MemberController {
    ...
    // [2] 로그인 + 세션(서버내 저장소 = 주로 로그인된 회원정보) : HttpServletRequest req
    @PostMapping("/login")
    ...
    public boolean login(@RequestBody MemberDto memberDto, HttpServletRequest req ){
        ...
        System.out.println("result = " + result);
        if (result == null){ // 서비스 결과가 null 이면 로그인 실패
            return false;
    ...
    public MemberDto info( HttpServletRequest request ){
        ...
        MemberDto memberDto = (MemberDto)object; // 4. 세션에 저장된 자료들은 모두 Object 타입 이므로 타입 변환한다.
